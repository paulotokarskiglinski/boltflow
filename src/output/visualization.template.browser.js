(function () {
  ...
  const btnFlowView = document.getElementById('btn-flow-view');
  const btnContainView = document.getElementById('btn-contain-view');
  const containmentLayer = document.getElementById('containment-layer');
  ...
  function renderGraph() {
    if (state.viewMode === 'containment') {
      renderContainmentView();
      return;
    }
    zonesGrp.innerHTML = '';
    edgesGrp.innerHTML = '';
    ...
    const DIM = '0.25'; // opacity for non-focused elements
  ...
  function renderContainmentView() {
    const vis = visibleSet();
    let html = '';
  ...
  function bindEvents() {
    ...
    // View mode toggles
    if (btnFlowView && btnContainView) {
      btnFlowView.addEventListener('click', () => {
        state.viewMode = 'flow';
        btnFlowView.classList.add('active');
        btnContainView.classList.remove('active');
        containmentLayer.classList.add('hidden');
        svg.style.display = 'block';
        document.querySelector('.zoom-btns').style.display = 'flex';
        legend.style.display = 'block';
        renderGraph();
        fitToScreen();
      });
      btnContainView.addEventListener('click', () => {
        state.viewMode = 'containment';
        btnContainView.classList.add('active');
        btnFlowView.classList.remove('active');
        containmentLayer.classList.remove('hidden');
        svg.style.display = 'none';
        document.querySelector('.zoom-btns').style.display = 'none';
    ...
    // Click on Containment View background → deselect
    if (containmentLayer) {
      containmentLayer.addEventListener('click', e => {
        if (e.target === containmentLayer) {
          closeDetail();
        }
      });
    }
